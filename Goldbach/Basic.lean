...
noncomputable def MinSymmetryOffset (n : ℕ)
    (h_exists : ∃ (k : ℕ), IsSymmetryOffset n k) : ℕ :=
  Nat.find h_exists

lemma mod_q_divisibility_obstacle (q n k : ℕ)
    (hn : q ∣ (n / 2)) (hk : q ∣ k) : q ∣ (n / 2 + k) :=
  dvd_add hn hk

lemma mod3_divisibility_obstacle (n k : ℕ)
    (hn : 3 ∣ (n / 2)) (hk : 3 ∣ k) : 3 ∣ (n / 2 + k) :=
  mod_q_divisibility_obstacle 3 n k hn hk

theorem goldbach_4 : ∃ k, IsSymmetryOffset 4 k := by
  use 0; dsimp [IsSymmetryOffset]; refine ⟨by decide, by decide⟩

theorem goldbach_6 : ∃ k, IsSymmetryOffset 6 k := by
  use 0; dsimp [IsSymmetryOffset]; refine ⟨by decide, by decide⟩

theorem goldbach_10 : ∃ k, IsSymmetryOffset 10 k := by
  use 2; dsimp [IsSymmetryOffset]; refine ⟨by decide, by decide⟩

-- ==========================================
-- PHASE 2: Structural Bounds & Parity
-- ==========================================
...
theorem prime_sum_reconstruction (n k : ℕ) (hn : IsValidEven n)
    (hk : IsSymmetryOffset n k) :
    (n / 2 - k) + (n / 2 + k) = n := by
...
lemma uneliminated_offsets_nonempty (n : ℕ) (_hn : IsValidEven n)
    (h8 : 8 ≤ n) : 0 < (UneliminatedOffsets n).card := by
  dsimp [UneliminatedOffsets]; rw [Finset.card_pos]
  by_cases h2 : 2 ∣ (n / 2)
  · use 1
    ...
    · intro h3; have : 3 ∣ 1 := h3.2; revert this; decide
  · by_cases h3 : 3 ∣ (n / 2)
    · use 2
      rw [Finset.mem_filter, Finset.mem_range]
      have h_bound : 2 < n / 2 - 1 := by
        ...
        have : n / 2 ≥ 9 := by
          have h_odd : m % 2 = 1 := by
            ...
            have : 2 ∣ (n / 2) := by
              rw [hm]; exact dvd_mul_of_dvd_right this 3
            contradiction
          have : m ≥ 3 := by omega
          omega
        omega
      refine ⟨h_bound, ⟨Or.inr ⟨h2, by decide⟩, ?_⟩⟩
      · intro h3_all; have : 3 ∣ 2 := h3_all.2; revert this; decide
    · use 0
      rw [Finset.mem_filter, Finset.mem_range]
      have h_bound : 0 < n / 2 - 1 := by
        ...
        omega
      refine ⟨h_bound, ⟨Or.inr ⟨h2, by decide⟩, ?_⟩⟩
      · intro h3_all; exact h3 h3_all.1

-- ==========================================
-- PHASE 4: Singular Series & the Analytic Bridge
-- ==========================================

/-- Twin Prime Constant C_2 -/
noncomputable def twinPrimeConstant : ℝ :=
  Real.exp (∑' p : { p : ℕ // Nat.Prime p ∧ p > 2 },
    Real.log (1 - 1 / ((p.val : ℝ) - 1) ^ 2))

/-- Singular Series S(n) arithmetic multiplier -/
noncomputable def singularSeries (n : ℕ) : ℝ :=
  if IsValidEven n then
    2 * twinPrimeConstant *
      ∏ p ∈ (Nat.properDivisors n).filter (fun x => Nat.Prime x ∧ x > 2),
        (((p : ℝ) - 1) / ((p : ℝ) - 2))
  else 0
...
/-- Stated as an ordinary Prop, NOT a Lean `axiom` — no proof
    term is ever supplied for it in this file. This is the single,
    explicitly open gap: equivalent in strength to the Goldbach
    Conjecture itself. -/
...
-- ==========================================
-- PHASE 5: Conditional Main Theorem
-- ==========================================

theorem goldbach_conjecture_from_bridge
    (h_bridge : AnalyticDensityBridge) :
    ∀ (n : ℕ), IsValidEven n → ∃ (k : ℕ), IsSymmetryOffset n k := by
  intro n hn
  have hn_gt2 : n > 2 := hn.1
  ...
  · have h8_ge : 8 ≤ n := by omega
    exact h_bridge n hn h8_ge
      (uneliminated_offsets_nonempty n hn h8_ge)
      (singular_series_lower_bound n hn)